...
export const legacySwitchRole = api<SwitchRoleRequest, SwitchRoleResponse>(
  { auth: true, expose: true, method: "POST", path: "/auth/legacy-switch-role" },
  async (req) => {
...
export const startFreelancerOnboarding = api<StartFreelancerOnboardingRequest, StartFreelancerOnboardingResponse>(
  { auth: true, expose: true, method: "POST", path: "/auth/start-freelancer-onboarding" },
  async (req) => {
    const auth = getAuthData()! as AuthData;

...
import { Search, MapPin, Sparkles, List, Map as MapIcon, TrendingUp } from "lucide-react";
...
// Mock data for trending stylists (in production, fetch from API)
const TRENDING_STYLISTS = [
  { id: "trending1", name: "Featured Pro", category: "braids", badge: "Top Rated" },
  { id: "trending2", name: "New Talent", category: "locs", badge: "Rising Star" },
];
...
export default function PublicDiscover() {
  ...
  const mapMarkers = results
    .filter((r) => r.freelancerPostcode) // Only include results with location
    .map((result) => ({
      id: result.id,
      lat: 51.5 + (Math.random() - 0.5) * 0.3, // Placeholder - in production use actual coordinates
                ...
                <div className="mb-6 flex items-center justify-between">
                  ...
                  {searchLocation && (
                    <span className="text-sm text-muted-foreground">
                      Within {filters.radiusMiles || 10} miles
                    </span>
                  )}
                </div>
